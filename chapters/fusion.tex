...
The are multiple approach of utilizing the Extended Kalman Filter to estimate position and orientation based on inertial sensor reading. They differ mainly in state vector---use the quaternion or euler angles to describe orientation, additionally tracks velocities and acceleration or extend the state vector of current bias of sensors. Also the additional sensors used in estimation may vary. 
%TODO: refs

\subsection{Constraints in multi-body systems}

Constraints are the limitations imposed on the system state. They can result from system’s structure, known trajectory or be directly linked with limitation of mathematical instrument used in the system description. In multi-body systems, a wide group of constraints is made of geometric constraints, i.e., the constraints that stem directly from the mechanism’s structure. They determine the directions in which the mechanism can move so as not to breach the limits imposed by the existence of kinematic pairs. The form of constraints may vary for the same mechanism, depending on the configuration it was mounted. The constraints are commonly non-linear and involve multiple elements from state vector. Generalizing, the constraints can be formulated as $\bm{c}(\bm{x}) = \bm{0}$. Provided that the constraints are continuously differentiable, the $\frac{\partial \bm{c}(\bm{x})}{\partial \bm{x}}$ is possible to derive. In a practical, the constraints are strictly linked with the structure of analyzed mechanism. As an example,  the selected constraint will be introduce: the quaternion norm constraint and fixation of yaw angle. \\
